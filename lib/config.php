...
function bigcloudcms_init_define_values(){
  global $bigcloudcms_premium; 
  if(isset($bigcloudcms_premium['post_word_count'])) { $excerptlength = $bigcloudcms_premium['post_word_count'];} else { $excerptlength = '40'; } 
  define('POST_EXCERPT_LENGTH', $excerptlength);
}

add_action('init', 'bigcloudcms_init_define_values');

function bigcloudcms_seo_switch() {
  global $bigcloudcms_premium;
   if(isset($bigcloudcms_premium['seo_switch'])) {
    if($bigcloudcms_premium['seo_switch'] == 1 ) {$useseo = true;} else { $useseo = false;}
} else {$useseo = true;}
  return $useseo;
  }
/**
 * .main classes
 */
function bigcloudcms_main_class() {
 ...
 * .sidebar classes
 */
function bigcloudcms_sidebar_class() {
  return 'col-md-4';
}
 ...
 */
function bigcloudcms_display_sidebar() {
   if (class_exists('woocommerce'))  {
        $sidebar_config = new Kadence_Sidebar(
        array('bigcloudcms_sidebar_on_product_post','bigcloudcms_sidebar_on_archive_page','bigcloudcms_sidebar_on_shop_page','bigcloudcms_sidebar_on_shop_cat_page','bigcloudcms_sidebar_on_blog_post','bigcloudcms_sidebar_on_blog_page', 'bigcloudcms_sidebar_on_staff_post','is_404','bigcloudcms_sidebar_on_home_page','bigcloudcms_sidebar_on_myaccount_page','is_cart','is_checkout',array('is_singular', array('portfolio')), array('is_tax', array('portfolio-type'))
        ),
  ...
  $sidebar_config = new Kadence_Sidebar(
    array('bigcloudcms_sidebar_on_blog_post','bigcloudcms_sidebar_on_archive_page_no_woo','bigcloudcms_sidebar_on_staff_post','bigcloudcms_sidebar_on_blog_page','is_404','bigcloudcms_sidebar_on_home_page', array('is_singular', array('portfolio')), array('is_tax', array('portfolio-type'))
      ),
    array('page-fullwidth.php','page-feature.php','page-portfolio.php','page-landing.php','page-staff-grid.php','page-testimonial-grid.php','page-contact.php', 'page-portfolio-category.php' )
  );
}

  return apply_filters('bigcloudcms_display_sidebar', $sidebar_config->display);
}

function bigcloudcms_sidebar_on_shop_page() {
  if( is_shop() ) {
  global $bigcloudcms_premium; 
    if(isset($bigcloudcms_premium['shop_layout']) && $bigcloudcms_premium['shop_layout'] == 'sidebar') {
        return false;
      } else {
        return true;
    }
...
function bigcloudcms_sidebar_on_shop_cat_page() {
  if(is_product_category() || is_product_tag()) {
  global $bigcloudcms_premium; 
    if(isset($bigcloudcms_premium['shop_cat_layout']) && $bigcloudcms_premium['shop_cat_layout'] == 'sidebar') {
        return false;
      } else {
        return true;
    }
...
function bigcloudcms_sidebar_on_product_post() {
  if( is_product()) {
     global $post;
    $postsidebar = get_post_meta( $post->ID, '_kad_post_sidebar', true );
      if(isset($postsidebar) && $postsidebar == 'yes') {
        ...
        global $bigcloudcms_premium; if(isset($bigcloudcms_premium['product_sidebar_default']) && $bigcloudcms_premium['product_sidebar_default'] == 'yes') {
            return false;
          } else  {
            return true;
          }
...
function bigcloudcms_sidebar_on_blog_post() {
  if(is_single() ) {
    global $post;
    $postsidebar = get_post_meta( $post->ID, '_kad_post_sidebar', true );
      if(isset($postsidebar) && $postsidebar == 'no') {
        return true;
          ...
          global $bigcloudcms_premium;
          if(isset($bigcloudcms_premium['blogpost_sidebar_default']) && $bigcloudcms_premium['blogpost_sidebar_default'] == 'no') {
            return true;
          } else {
            return false;
          }
...
function bigcloudcms_sidebar_on_staff_post() {
...
function bigcloudcms_sidebar_on_home_page() {
  if(is_front_page()) {
      global $bigcloudcms_premium; 
      if(isset($bigcloudcms_premium['home_sidebar_layout']) && $bigcloudcms_premium['home_sidebar_layout'] == 'sidebar') {
        return false;
        } else {
          return true;
        }
...
function bigcloudcms_sidebar_on_myaccount_page() {
  if(is_account_page()) {
...
function bigcloudcms_sidebar_on_archive_page() {
  if(is_archive() && !is_shop() && !is_product_category() && !is_product_tag()) {
      global $bigcloudcms_premium; 
      if(isset($bigcloudcms_premium['blog_cat_layout']) && $bigcloudcms_premium['blog_cat_layout'] == 'sidebar') {
        return false;
        } else {
          return true;
        }
...
function bigcloudcms_sidebar_on_archive_page_no_woo() {
  if(is_archive()) {
      global $bigcloudcms_premium; 
      if(isset($bigcloudcms_premium['blog_cat_layout']) && $bigcloudcms_premium['blog_cat_layout'] == 'sidebar') {
        return false;
...
function bigcloudcms_sidebar_on_blog_page() {
...
function bigcloudcms_shop_layout_css() {
  global $bigcloudcms_premium;
  if(bigcloudcms_display_sidebar()) {
          if(isset($bigcloudcms_premium['product_shop_layout'])) {
            $columns = "shopcolumn".$bigcloudcms_premium['product_shop_layout']." shopsidebarwidth"; 
          } else {$columns = "shopcolumn4 shopsidebarwidth"; }
      } else {
         if(isset($bigcloudcms_premium['product_shop_layout'])) { $columns = "shopcolumn".$bigcloudcms_premium['product_shop_layout']." shopfullwidth"; 
          } else { $columns = "shopcolumn4 shopfullwidth";  }
      }

  return $columns;
}
function bigcloudcms_category_layout_css() {
  global $bigcloudcms_premium;
  if(bigcloudcms_display_sidebar()) {
          if(isset($bigcloudcms_premium['product_shop_layout']) && $bigcloudcms_premium['product_shop_layout'] == "single") {
...
function bigcloudcms_display_topbar() {
  global $bigcloudcms_premium;
   if(isset($bigcloudcms_premium['topbar'])) {
  if($bigcloudcms_premium['topbar'] == 1 ) {$topbar = true;} else { $topbar = false;}
} else {$topbar = true;}
  return $topbar;
  }
function bigcloudcms_display_topbar_icons() {
  global $bigcloudcms_premium;
 if(isset($bigcloudcms_premium['topbar_icons'])) {
  if($bigcloudcms_premium['topbar_icons'] == 1 ) {$topbaricons = true;} else { $topbaricons = false;}
} else {$topbaricons = false;}
  return $topbaricons;
  }
  function bigcloudcms_display_top_search() {
  global $bigcloudcms_premium;
 if(isset($bigcloudcms_premium['topbar_search'])) {
  if($bigcloudcms_premium['topbar_search'] == 1 ) {$topsearch = true;} else { $topsearch = false;}
} else {$topsearch = true;}
  return $topsearch;
  }
function bigcloudcms_display_topbar_widget() {
  global $bigcloudcms_premium;
 if(isset($bigcloudcms_premium['topbar_widget'])) {
  if($bigcloudcms_premium['topbar_widget'] == 1 ) {$topbarwidget = true;} else { $topbarwidget = false;}
...
function layout_class_names($classes) {
 global $bigcloudcms_premium;
  ...
  if(isset($bigcloudcms_premium['boxed_layout'])) {
    $layoutstyle = $bigcloudcms_premium['boxed_layout'];
  } else {
    $layoutstyle = 'wide';
  }
...
function layout_sticky_class($classes) {
 global $bigcloudcms_premium;
  // add 'class-name' to the $classes array
  if(isset($bigcloudcms_premium['sticky_header']) && $bigcloudcms_premium['sticky_header'] == '1') {
...
return apply_filters('bigcloudcms_icon_list', $icons);
